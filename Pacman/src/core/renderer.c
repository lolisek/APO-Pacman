...
void render_init(void)
{
    ...
    wall_texture = load_ppm("/tmp/veru/assets/resources/block.ppm");
    pellet_texture = load_ppm("/tmp/veru/assets/resources/pellet.ppm");
    power_pellet_texture = load_ppm("/tmp/veru/assets/resources/power-pellet.ppm");
    ...
    // Pacman textures
    pacman_textures[0] = load_ppm("/tmp/veru/assets/resources/pacman-right.ppm");
    pacman_textures[1] = load_ppm("/tmp/veru/assets/resources/pacman-left.ppm");
    pacman_textures[2] = load_ppm("/tmp/veru/assets/resources/pacman-up.ppm");
    pacman_textures[3] = load_ppm("/tmp/veru/assets/resources/pacman-down.ppm");
    pacman_textures[4] = load_ppm("/tmp/veru/assets/resources/pacman-closed.ppm");
    if (!pacman_textures[0] || !pacman_textures[1] || !pacman_textures[2] 
        || !pacman_textures[3] || !pacman_textures[4])
    {
    ...
    // Ghost textures
    ghost_red_textures[0] = load_ppm("/tmp/veru/assets/resources/ghost-red-right.ppm");
    ghost_red_textures[1] = load_ppm("/tmp/veru/assets/resources/ghost-red-left.ppm");
    ghost_orange_textures[0] = load_ppm("/tmp/veru/assets/resources/ghost-or-right.ppm");
    ghost_orange_textures[1] = load_ppm("/tmp/veru/assets/resources/ghost-or-left.ppm");
    ghost_blue_textures[0] = load_ppm("/tmp/veru/assets/resources/ghost-blue-right.ppm");
    ghost_blue_textures[1] = load_ppm("/tmp/veru/assets/resources/ghost-blue-left.ppm");
    ghost_pink_textures[0] = load_ppm("/tmp/veru/assets/resources/ghost-pink-right.ppm");
    ghost_pink_textures[1] = load_ppm("/tmp/veru/assets/resources/ghost-pink-left.ppm");
...
void render_pacman(const Entity *pacman, uint16_t *fb, int animation_frame, int offset_x, int offset_y) {
    int direction_index;
    if (pacman->direction.y == 0 && pacman->direction.x == 1) direction_index = 0; // Right
    else if (pacman->direction.y == 0 && pacman->direction.x == -1) direction_index = 1; // Left
    else if (pacman->direction.y == -1 && pacman->direction.x == 0) direction_index = 2; // Up
    else direction_index = 3; // Down
